---
layout: page
title: Projects
---

# Projects

Below are selected data analytics and visualization projects showcasing my work.

---

## Sales Performance Dashboard

**Tools:** Tableau, Excel  
**Focus:** Revenue trends and regional performance

![Sales Dashboard](assets/sales-dashboard.png)

**Key Insights:**
- Revenue increased 18% quarter-over-quarter
- The West region underperformed compared to the national average
- High-growth products drove most gains

[View interactive dashboard](LINK_HERE)

---

## Customer Segmentation Analysis

**Tools:** Power BI  
**Focus:** Behavioral segmentation and retention

![Customer Segmentation](assets/customer-segmentation.png)

**Key Insights:**
- Identified 3 distinct customer segments
- High-value segment showed strongest retention
- Churn risk concentrated in one segment

[View report](LINK_HERE)

